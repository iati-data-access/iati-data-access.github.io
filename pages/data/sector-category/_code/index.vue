<template>
  <div>
    <DataBrowserNavbar />
    <h1>
      <span class="text-muted">{{ $t('dataDashboards.sectors.for') }}</span> <b>{{ drilldownLabel }}</b>
    </h1>
    <hr />
    <b-row>
      <b-col>
...
export default {
  ...
  head() {
    return {
      title: `${this.drilldownLabel} - ${this.$t('dataDashboards.sectors.by')} - ${this.$t('dataDashboards.label')} - ${this.$t('title')}`,
      meta: [
        {
          hid: 'description',
          name: 'description',
